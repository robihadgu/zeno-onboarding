...
export default function PipelineView({ clients, onRefresh }: Props) {
  ...
  function getActionButton(client: Client) {
    ...
    // Agreement sent — manual override to mark signed
    ...
    // Welcome sent — show BOTH buttons (payment + onboarding can happen in any order)
    ...
    // Payment confirmed but onboarding not done yet
    if (client.status === "payment_confirmed") {
      return (
        <div className="space-y-1.5">
          <div className="text-[10px] text-center py-1 rounded-md" style={{ background: "rgba(34,197,94,0.06)", color: "var(--green)" }}>
            Payment &#10003;
          </div>
          <button onClick={() => advanceClient(client, "onboarding_complete")} className="w-full text-[11px] py-2 font-medium cursor-pointer btn-ghost">
            Mark Onboarded
    ...
    // Onboarding done but payment not confirmed yet
      ...
      <div className="flex items-center gap-1.5 mt-2">
        {total > 0 && (
          <span
            ...
            style={{ background: "rgba(234,179,8,0.06)", color: "#EAB308", border: "1px solid rgba(234,179,8,0.1)" }}
          >
    ...
    <div>
      ...
      {clients.length === 0 ? (
        ...
        >
          ...
          <p className="text-[12px] mt-1" style={{ color: "var(--foreground-dim)" }}>Start an onboarding above to see clients in the pipeline</p>
        </div>
      ) : (
                    ...
                    >
                      ...
                      {deleteConfirm === client.id ? (
                        <div
                          ...
                          style={{ background: "rgba(2,2,3,0.95)", border: "1px solid rgba(239,68,68,0.2)" }}
                        >
                      ...
                      <div className="text-[10px] mt-0.5 truncate" style={{ color: "var(--foreground-dim)" }}>
                        {client.email}
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <span
                          ...
                          style={{ background: "var(--surface)", color: "var(--foreground-dim)" }}
                        >
                          {client.plan}
                        </span>
                      </div>
